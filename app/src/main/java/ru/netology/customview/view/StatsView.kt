package ru.netology.customview.view
...
class StatsView @JvmOverloads constructor(
    ...
    init {
        ...
        context.withStyledAttributes(attrs, R.styleable.StatsView) {
            ...
            colors = listOf(
                ...
                getColor(R.styleable.StatsView_color6, randomColor()),
                getColor(R.styleable.StatsView_color7, randomColor()),
                getColor(R.styleable.StatsView_color8, randomColor()),
                getColor(R.styleable.StatsView_color9, randomColor()),
                getColor(R.styleable.StatsView_color10, randomColor()),
            )
        }
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for ((index, datum) in data.withIndex()) {
            val angleCurrent = min(360F * (datum / sumValues), maxAngle)
            paint.color = colors.getOrNull(index) ?: randomColor()
            canvas.drawArc(oval, startAngle, angleCurrent, false, paint)

            paint.color = colors[0]
            canvas.drawArc(oval, -90F, 1F, false, paint)

            startAngle += angleCurrent
        }
